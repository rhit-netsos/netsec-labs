...
# list of hosts in the docker compose file
HOSTS="browser firewall server workstation"
print_log "Patching hosts: ${HOSTS}"
for hhost in ${HOSTS}
do
  PATTERN="s/${hhost}/${STUDENT}-${hhost}/g"
...
# patching the firewall rules
print_log "Patching firewall rules..."
PATTERN="s/10.10/${SUBNET}/g"
sed -i $PATTERN volumes/firewall/rules.nft

# generate connection scripts
print_log "Generating connection scripts..."
CONN_HOSTS="browser"
for hhost in ${CONN_HOSTS}
do
